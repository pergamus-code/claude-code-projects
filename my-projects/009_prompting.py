...
class PromptEvaluator:
    ...
    def generate_unique_ideas(self, task_description, prompt_inputs_spec, num_cases):
        ...
        system_prompt = "You are a test scenario designer specialized in creating diverse, unique testing scenarios. Output only json"
        ...
        text = chat(messages, system=system_prompt, temperature=1.0)
        clean_text = stripper(text)
    ...
    def generate_test_case(self, task_description, idea, prompt_inputs_spec={}):
        ...
        system_prompt = "You are a test case creator specializing in designing evaluation scenarios. Output json only."
        ...
        messages = []
        add_user_message(messages, rendered_prompt)
        text = chat( messages, system=system_prompt, temperature=0.7)
        clean_text = stripper(text)
        test_case = json.loads(clean_text)
    ...
    def grade_output(self, test_case, output, extra_criteria):
        ...
        * The full 1-10 scale should be utilized - don't hesitate to give low scores when warranted
        ...
        eval_text = chat( messages, temperature=0.0, system="Output json only")
        clean_text = stripper(eval_text)
        return json.loads(clean_text)
...
dataset = evaluator.generate_dataset(
    ...
    prompt_inputs_spec={
        "height": "Ahtlete's height in cm",
        "weight": "Ahtlete's weight in kg",
...
def run_prompt(prompt_inputs):
    ...
    height: 178
    weight: 72
    goal: Match day performance and recovery for soccer
    restrictions: Vegan diet
    </sample_input>

    <ideal_output>
    # One-Day Vegan Match Day Meal Plan
    **Soccer Player | 178cm | 72kg**
    ...
    - **Calories:** 3,200 kcal
    - **Protein:** 130g (16%)
    - **Carbohydrates:** 420g (53%)
    - **Fat:** 85g (24%)

    ---

    ## BREAKFAST (7:00 AM) | 750 kcal
    **Oatmeal Power Bowl**
    - Rolled oats: 80g
    - Plant-based milk (soy): 300ml
    - Banana: 150g
    - Almond butter: 30g
    - Ground flaxseed: 15g
    - Blueberries: 80g

    **Macros:** 28g protein | 95g carbs | 22g fat
    ...
    **Macros:** 12g protein | 62g carbs | 10g fat

    ---

    ## LUNCH (1:00 PM) | 850 kcal
    **Chickpea & Quinoa Buddha Bowl**
    - Cooked quinoa: 200g
    - Canned chickpeas: 180g
    ...
    **Macros:** 32g protein | 96g carbs | 28g fat

    ---

    ## PRE-MATCH SNACK (3 hours before game) | 450 kcal
    **Easy Digestion Blend**
    - White rice cakes: 50g
    - Peanut butter: 25g
    - Honey: 30g
    - Apple: 180g

    **Macros:** 10g protein | 65g carbs | 12g fat

    ---

    ## POST-MATCH RECOVERY (Within 30 mins) | 500 kcal
    **Recovery Smoothie**
    - Plant-based protein powder: 30g
    - Banana: 150g
    - Coconut milk (canned): 100ml
    - Spinach: 50g
    - Chia seeds: 15g
    - Rice syrup: 20g

    **Macros:** 28g protein | 62g carbs | 12g fat

    ---

    ## DINNER (7:00 PM) | 250 kcal
    **Light Recovery Meal**
    - Lentil pasta: 100g (cooked)
    - Marinara sauce: 150g
    - Broccoli: 150g
    - Olive oil: 10ml
    - Nutritional yeast: 10g

    **Macros:** 20g protein | 40g carbs | 1g fat

    ---

    ## Key Performance Features
    ✅ **High carbs** for glycogen storage and match intensity  
    ✅ **Adequate protein** from varied plant sources for muscle repair  
    ✅ **Easy digestion** before match (white rice, simple sugars)  
    ✅ **Recovery focus** post-match (carbs + protein ratio)  
    ✅ **Budget-friendly** staples (oats, beans, lentils, seasonal produce)  
    ✅ **Hydration reminder:** 3-4L water + electrolyte drink during match

    ---

    **Meal timing optimized for afternoon/evening match day**
    </ideal_output>

    The solution comprehensively satisfies all three mandatory requirements: daily caloric total is clearly stated (3,200 kcal),
    macronutrient breakdown is provided both as percentages and absolute grams for each meal, and all meals include exact foods with portions and timing.
    """
