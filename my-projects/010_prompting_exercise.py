...
# DEFINITION: THIS IS WHAT PROMP EVALUATION PIPELINE LOOKS LIKE: create dataset of test cases catered to our main prompt, run the test cases through a grader and spit out a score to score our output of the main PROMPT
...
### THIS ONE HELPS WITH THE PROMT TO CREATE THE TEST CASES ### ### ### ### ### ### ### ### ###
...
### GENERATE TEST CASES USING THE IDEAS FOR A TESTCASE FROM THE FUNCTION ABOVE ###
...
### RUN TEST CASE GOES TOGETHER WITH THE GRADER ### BECAUSE TO SCORE THE TEST CASE WE FIRST NEED TO RUN IT (TAKE IT) THEN TEST IT (GRADE IT) ### ### ### ### ### ### 
...
## RUN EVAL STARTS THE WHOLE PROCES OF RUNNING THE EVAL PIPELINE FROM RUN TEST TO GRADING THEM EACH THEN PROVIDING THE AVERAGE OF EACH ##
...
# %%
# DEFINING OUR MAIN PROMPT - THIS IS WHERE PROMT ENG GOES INTO
def run_prompt(prompt_inputs):
    prompt = f"""
    Extract key topics mentioned from a passage of text from a scholarly journal into a JSPN array of strings. 
